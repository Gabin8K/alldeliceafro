...
const isOpenEditShopModal = ref(false)
const images = ref([]);
const max_upload = ref(MAX_IMAGES);

const isOpenDeleteImageModal = ref(false)
const toDeletedImageId = ref(null)

const isOpenDeleteShopModal = ref(false)
...
const UpdateShopImageFilePond = vueFilePond(
...
function openEditShopModal() {
    axios.get('shops/' + props.id)
        .then(response => {
            form.name = response.data.name
            form.description_en = response.data.description_en
            form.description_fr = response.data.description_fr
            form.description_de = response.data.description_de
            images.value = response.data.image ? [response.data.image] : []
            max_upload.value -= response.data.image ? [response.data.image].length : 0
            isOpenEditShopModal.value = true
        })
}

function eventEditShopModalClose() {
    form.reset();
    form.clearErrors();
    images.value = [];
    max_upload.value = MAX_IMAGES;
...
function deleteImage() {
    ...
    max_upload.value = MAX_IMAGES - images.value.length;
...
function deleteShop() {
    router.delete(route('admin.shops.destroy', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length
    });
    isOpenDeleteShopModal.value = false;
}

function updateShop() {
    form.patch(route('admin.shops.update', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length,
        onSuccess: () => isOpenEditShopModal.value = false
    });
}
...
<template>
    <div class="flex items-center justify-end gap-x-2 md:gap-x-4">
        <Link
            :href="route('admin.shops.edit', [$page.props.currentLocale, id])"
        ...
        <div @click="isOpenDeleteShopModal = true"
    ...
    <MazDialog
        v-model="isOpenEditShopModal"
        title="Edit shop"
        width="800px"
        maxHeight="80vh"
        scrollable
        @close="eventEditShopModalClose()"
    >
        <form @submit.prevent="updateShop" class="space-y-6">
            ...
            <div v-if="images.length > 0" class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div v-for="image in images" class="relative rounded-xl overflow-hidden hover:scale-105 transform transition duration-200 w-fit">
                    <div @click="openDeleteImageModal(image.id)" class="absolute top-2 right-2 bg-red-500 rounded-full p-1 cursor-pointer hover:scale-105">
            ...
            <UpdateShopImageFilePond
                ...
                :required="max_upload >= MAX_IMAGES"
