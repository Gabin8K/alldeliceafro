...
const isOpenEditArticleModal = ref(false)
const images = ref([]);
...
const isOpenDeleteArticleModal = ref(false)
...
const UpdateArticleImageFilePond = vueFilePond(
...
function openEditArticleModal() {
    axios.get('articles/' + props.id)
        .then(response => {
            form.category_id = response.data.category_id
            form.name_en = response.data.name_en
            form.name_fr = response.data.name_fr
            form.name_de = response.data.name_de
            ...
            form.weight = response.data.weight
            images.value = response.data.images
            max_upload.value -= response.data.images.length
            isOpenEditArticleModal.value = true
        })
}

function eventEditArticleModalClose() {
    form.reset();
    form.clearErrors();
    images.value = [];
...
function deleteArticle() {
    router.delete(route('admin.articles.destroy', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length
    });
    isOpenDeleteArticleModal.value = false;
}

function updateArticle() {
    form.patch(route('admin.articles.update', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length,
        onSuccess: () => isOpenEditArticleModal.value = false
    });
}
...
<template>
    <div class="flex items-center justify-end gap-x-2 md:gap-x-4">
        <div @click="openEditArticleModal()"
        ...
        <div @click="isOpenDeleteArticleModal = true"
    ...
    <MazDialog
        v-model="isOpenDeleteArticleModal"
    ...
    <MazDialog
        v-model="isOpenEditArticleModal"
        title="Edit article"
        width="800px"
        maxHeight="80vh"
        scrollable
        @close="eventEditArticleModalClose()"
    >
