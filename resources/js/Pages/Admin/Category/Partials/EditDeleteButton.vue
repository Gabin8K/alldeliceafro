...
const UpdateCategoryImageFilePond = vueFilePond(
...
function eventEditCategoryModalClose() {
    form.reset();
    form.clearErrors();
    images.value = [];
...
function deleteCategory() {
    router.delete(route('admin.categories.destroy', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
...
function updateCategory() {
    form.patch(route('admin.categories.update', [usePage().props.currentLocale, props.id]), {
        preserveScroll: true,
        preserveState: (page) => Object.keys(page.props.errors).length,
        onSuccess: () => isOpenEditCategoryModal.value = false
    });
}
...
<template>
    <div class="flex items-center justify-end gap-x-2 md:gap-x-4">
        <div @click="openEditCategoryModal()"
        ...
        <div @click="isOpenDeleteCategoryModal = true"
    ...
    <MazDialog
        v-model="isOpenDeleteCategoryModal"
    ...
    <MazDialog
        v-model="isOpenEditCategoryModal"
        title="Edit category"
        width="800px"
        maxHeight="80vh"
        scrollable
        @close="eventEditCategoryModalClose()"
    >
